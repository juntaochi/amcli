...
impl MediaPlayer for AppleMusicController {
    ...
    async fn get_player_status(&self) -> Result<PlayerStatus> {
        let script = r#"
            tell application "Music"
                ...
                if pState is not "stopped" then
                    ...
                    return pState & ":::BOLT_SPLIT:::" & vol & ":::BOLT_SPLIT:::" & tName & ":::BOLT_PART:::" & tArtist & ":::BOLT_PART:::" & tAlbum & ":::BOLT_PART:::" & tDuration & ":::BOLT_PART:::" & tPosition
                else
        ...
        let volume = volume_str.parse().unwrap_or(0);

        let track = if !track_str.is_empty() && state != PlaybackState::Stopped {
            let track_parts: Vec<&str> = track_str.split(":::BOLT_PART:::").collect();
...
mod tests {
    ...
    async fn test_get_player_status() {
        let mut mock = MockCommandRunner::new();
        let output = "playing:::BOLT_SPLIT:::80:::BOLT_SPLIT:::Song:::BOLT_PART:::Artist:::BOLT_PART:::Album:::BOLT_PART:::200.0:::BOLT_PART:::100.0";
        mock.expect_execute()
            .times(1)
            .returning(move |_| Ok(mock_output(output, true)));
