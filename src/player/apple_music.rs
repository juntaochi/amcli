...
impl MediaPlayer for AppleMusicController {
    ...
    async fn get_artwork_url(&self, track: &Track) -> Result<Option<String>> {
        ...
        // ⚡ Bolt Optimization:
        // By cloning and reusing a shared `reqwest::Client` instead of using the stateless `reqwest::get`,
        // we leverage HTTP Keep-Alive and internal connection pooling. This eliminates the massive
        // overhead of establishing a new TCP connection and performing a full TLS handshake for
        // every single background artwork fetch.
        ...
        tokio::spawn(async move {
            ...
            if let Ok(Ok(response)) =
                tokio::time::timeout(timeout_duration, client.get(&url).send()).await
            {
                if let Ok(Ok(json)) =
