...
impl MediaPlayer for AppleMusicController {
    ...
    async fn get_player_status(&self) -> Result<PlayerStatus> {
        // Bolt ⚡ Optimization:
        // osascript process spawns are extremely expensive (100-200ms per call).
        // This batch AppleScript execution retrieves state, volume, and track metadata
        // simultaneously using a custom flat-data delimiter (:::BOLT_SPLIT:::).
        // This cuts IPC overhead in half during the high-frequency UI update loop.
        let script = r#"
            tell application "Music"
        ...
        let result = self.execute_script(script).await?;
        if result.is_empty() {
            return Err(anyhow!("Empty status response"));
        }
