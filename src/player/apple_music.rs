    ...
    async fn get_player_status(&self) -> Result<PlayerStatus> {
        let script = r#"
            tell application "Music"
                ...
                if pState is not "stopped" then
                    ...
                    return vol & ":::BOLT_SPLIT:::" & pState & ":::BOLT_SPLIT:::" & tName & ":::BOLT_PART:::" & tArtist & ":::BOLT_PART:::" & tAlbum & ":::BOLT_PART:::" & tDur & ":::BOLT_PART:::" & tPos
                else
        ...
        let track = if state != PlaybackState::Stopped && parts.len() >= 3 {
            ...
            Some(Track {
                ...
                duration: Duration::from_secs_f64(track_parts[3].parse()?),
                position: Duration::from_secs_f64(track_parts[4].parse()?),
            })
        } else {
            None
        };
...
mod tests {
    ...
    async fn test_get_player_status() {
        let mut mock = MockCommandRunner::new();
        // format: vol:::BOLT_SPLIT:::state:::BOLT_SPLIT:::name:::BOLT_PART:::artist:::BOLT_PART:::album:::BOLT_PART:::duration:::BOLT_PART:::position
    ...
    async fn test_get_player_status_stopped() {
        let mut mock = MockCommandRunner::new();
        let output = "50:::BOLT_SPLIT:::stopped";
        mock.expect_execute()
            .times(1)
            .returning(move |_| Ok(mock_output(output, true)));
