...
impl MediaPlayer for AppleMusicController {
    ...
    async fn get_player_status(&self) -> Result<PlayerStatus> {
        let script = r#"
            tell application "Music"
                ...
                if p_state is not "stopped" then
                    set t_info to name of current track & ":::BOLT_SPLIT:::" & ¬
                ...
                return p_state & ":::BOLT_SPLIT:::" & v & ":::BOLT_SPLIT:::" & t_info
            end tell
        "#;
        ...
        let track = if parts.len() >= 7 && state != PlaybackState::Stopped {
            Some(Track {
...
mod tests {
    ...
    async fn test_get_player_status_playing() {
        let mut mock = MockCommandRunner::new();
        // Output format: state:::BOLT_SPLIT:::volume:::BOLT_SPLIT:::name:::BOLT_SPLIT:::artist:::BOLT_SPLIT:::album:::BOLT_SPLIT:::duration:::BOLT_SPLIT:::position
        let output = "playing:::BOLT_SPLIT:::50:::BOLT_SPLIT:::Song:::BOLT_SPLIT:::Artist:::BOLT_SPLIT:::Album:::BOLT_SPLIT:::120:::BOLT_SPLIT:::10";
        ...
        assert_eq!(track.duration.as_secs(), 120);
    }

    #[tokio::test]
    async fn test_get_player_status_stopped() {
        let mut mock = MockCommandRunner::new();
        // Output format: stopped:::BOLT_SPLIT:::50:::BOLT_SPLIT:::
        let output = "stopped:::BOLT_SPLIT:::50:::BOLT_SPLIT:::";
