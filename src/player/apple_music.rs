            ...
            tell application "Music"
                ...
                if pState is not "stopped" then
                    set trackInfo to name of current track & ":::BOLT_SPLIT:::" & ¬
                                     ...
                                     player position
                    return vol & ":::BOLT_SPLIT:::" & pState & ":::BOLT_SPLIT:::" & trackInfo
                else
...
mod tests {
    ...
    async fn test_get_player_status() {
        let mut mock = MockCommandRunner::new();
        let output = "50:::BOLT_SPLIT:::playing:::BOLT_SPLIT:::Song:::BOLT_SPLIT:::Artist:::BOLT_SPLIT:::Album:::BOLT_SPLIT:::200.0:::BOLT_SPLIT:::100.0";
        mock.expect_execute()
            .times(1)
            .returning(move |_| Ok(mock_output(output, true)));
