...
pub trait MediaPlayer: Send + Sync {
    ...
    async fn get_player_status(&self) -> Result<PlayerStatus> {
        let (track_result, volume_result, state_result) =
            tokio::join!(self.get_current_track(), self.get_volume(), self.get_playback_state());

        Ok(PlayerStatus {
            track: track_result.ok().flatten(),
            volume: volume_result.unwrap_or(50),
            state: state_result.unwrap_or(PlaybackState::Stopped),
        })
    }
}
