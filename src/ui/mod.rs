...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    // Bolt ⚡ Optimization:
    // Use a static slice `&[(&str, &str)]` with pre-formatted text (including spaces and brackets)
    // instead of dynamic `vec![]` and `format!` macros.
    // This avoids ~15 heap allocations per frame in the high-frequency UI draw loop.
