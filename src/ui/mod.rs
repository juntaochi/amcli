...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    // Bolt ⚡ Optimization:
    // We use a static array of pre-formatted string slices (including spaces and brackets)
    // instead of a `vec!` and `format!` inside the loop. This avoids ~14 allocations
    // per frame during the high-frequency TUI `draw` loop.
