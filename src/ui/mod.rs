...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    // Bolt ⚡ Optimization:
    // We use a static array slice instead of `vec![]` allocations on every `draw` tick.
    // Furthermore, string formatting (padding spaces and brackets) is baked into the literals
    // rather than using `format!` dynamically inside the loop. This saves ~14 heap allocations per frame.
