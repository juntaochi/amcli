...
impl App {
    ...
    pub async fn update(&mut self) -> Result<()> {
        ...
        if let Some(ref track) = self.current_track {
            ...
            // Optimized: Inline duration formatting directly into the final format! macros.
            // This prevents nested format! calls which would create intermediate String allocations.
            // In a high-frequency UI update loop (~500ms), eliminating these extra heap allocations
            // reduces memory churn and CPU overhead during string concatenation.
