...
impl App {
    ...
    pub async fn update(&mut self) -> Result<()> {
        ...
        if let Some(ref track) = self.current_track {
            ...
            // ⚡ Bolt Optimization:
            // Inline time formatting to eliminate intermediate String allocations from helper functions.
            // Avoids nested format! calls, speeding up the hot TUI update loop path.
            let p_secs = track.position.as_secs();
            let d_secs = track.duration.as_secs();
            ...
            cache.gauge_label = format!(" {}s/{}s | {:02}% ", p_secs, d_secs, progress_percent);
