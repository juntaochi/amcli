...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    // Bolt ⚡ Optimization:
    // Pre-formatting button labels and keys as static string slices avoids ~14 heap
    // allocations (vec! and format!) per frame during the TUI hot render loop.
    ...
    for _ in 0..controls.len() {
        constraints.push(Constraint::Length(btn_width));
    }
