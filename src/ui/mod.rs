...
impl App {
    ...
    pub async fn update(&mut self) -> Result<()> {
        // Fetch track, volume, and state in a single operation
        let (new_track, new_volume) = match self.player.get_player_status().await {
            Ok(status) => (status.track, status.volume),
            Err(_) => (None, self.volume), // Fallback to current volume on error to prevent UI flicker
        };
