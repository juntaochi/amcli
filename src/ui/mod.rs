...
impl App {
    ...
    pub async fn update(&mut self) -> Result<()> {
        ...
        if let Some(ref track) = self.current_track {
            ...
            let progress_percent = if dur_secs > 0 {
            ...
            // ⚡ Bolt Optimization:
            // Inline duration calculations directly into the single `format!` macro to eliminate
            // intermediate string allocations that were created by `format_duration` and
            // `format_duration_seconds`. This cuts formatting overhead by nearly 50% in the
