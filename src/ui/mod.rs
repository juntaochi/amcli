...
fn draw_progress(
    ...
    let (progress_percent, label) = if let Some(c) = cache {
        (c.progress_percent, Cow::Borrowed(c.gauge_label.as_str()))
    } else {
        let pct = if track.duration.as_secs() > 0 {
            ((track.position.as_secs_f64() / track.duration.as_secs_f64()) * 100.0) as u16
        } else {
            0
        };
...
fn draw_metadata(
    ...
    let values = if let Some(c) = cache {
        [
            Cow::Borrowed(c.name.as_str()),
            Cow::Borrowed(c.artist.as_str()),
            Cow::Borrowed(c.album.as_str()),
            Cow::Borrowed(c.duration_str.as_str()),
        ]
    } else {
        [
