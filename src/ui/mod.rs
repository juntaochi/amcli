...
impl App {
    ...
    pub async fn update(&mut self) -> Result<()> {
        ...
        if let Some(ref track) = self.current_track {
            ...
            // Previously, `format_duration` and `format_duration_seconds` created temporary Strings
            // that were then interpolated into another `format!`. Doing it in one step avoids ~50%
            // of formatting overhead in the TUI's high-frequency update loop.
