...
impl App {
    ...
    pub async fn update(&mut self) -> Result<()> {
        // Optimization: Use get_player_status to fetch track, volume, and state in a single IPC call.
        // This reduces overhead significantly compared to separate calls.
        let (new_track, volume) = match self.player.get_player_status().await {
            Ok(status) => (status.track, status.volume.unwrap_or(self.volume)),
