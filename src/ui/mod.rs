...
impl App {
    ...
    pub async fn update(&mut self) -> Result<()> {
        ...
        if let Some(ref track) = self.current_track {
            ...
            // Optimized: Inlined duration math to eliminate 4 intermediate String allocations per tick
            let pos_secs = track.position.as_secs();
            let pos_mins = pos_secs / 60;
            let pos_rem = pos_secs % 60;

            let dur_secs = track.duration.as_secs();
            let dur_mins = dur_secs / 60;
            let dur_rem = dur_secs % 60;

            cache.duration_str = format!(
                "{:02}:{:02} / {:02}:{:02}",
                pos_mins, pos_rem, dur_mins, dur_rem
            );
